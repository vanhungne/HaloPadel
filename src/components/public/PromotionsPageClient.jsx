...
export default function PromotionsPageClient({ promotions = [] }) {
  ...
  const getBadges = (index) => {
    if (index === 0) return [t.promotions.discount30, t.promotions.freeDrinks, t.promotions.freeRacket];
    return [t.promotions.save25, t.promotions.groupFriends];
  }
    ...
    <div className="w-full px-4 md:px-8 max-w-[1200px] mx-auto">
      ...
      <div className="text-center max-w-3xl mx-auto mb-16">
        ...
        <h1 className="font-heading text-4xl md:text-5xl lg:text-[56px] font-bold text-[#111111] mb-4">
          {t.promotions.pageTitle} <span className="text-[#D45A2A]">{t.promotions.pageTitleHighlight}</span>
        </h1>
        <p className="text-[#555555] text-lg">{t.promotions.pageSubtitle}</p>
      </div>
      ...
      {promotions.length > 0 ? (
                ...
                <div key={promo.id} className={`group bg-white rounded-[32px] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-[#E8E2D2] transition-all hover:shadow-[0_20px_60px_rgba(212,90,42,0.15)] hover:border-[#D45A2A]/30 flex flex-col lg:flex-row ${isExpired ? 'opacity-70 grayscale-[50%]' : ''}`}>
                  ...
                  <div className="w-full lg:w-[45%] p-8 lg:p-12 flex flex-col justify-center">
                    <div className="flex flex-wrap gap-2 mb-6">
                      <span className="bg-[#FFF9EE] text-[#D45A2A] border border-[#D45A2A]/20 px-3 py-1 rounded-lg text-[13px] font-semibold">{t.promotions.opening}</span>
                      {promo.startDate && promo.endDate && (
                        <span className="bg-[#F8F5E4] text-[#555555] px-3 py-1 rounded-lg text-[13px] font-semibold">
                          {t.promotions.until} {formatDate(promo.endDate)}
                        </span>
                      )}
                    </div>
                    ...
                    {promo.shortDesc && (
                      <p className="text-[#555555] text-lg mb-6 leading-relaxed">
                        {localize(promo, 'shortDesc', locale)}
                      </p>
                    )}
            ...
            return (
              ...
              >
                <div className="relative w-full md:w-[35%] h-[240px] md:h-auto overflow-hidden">
                  ...
                  {isExpired && (
                    <div className="absolute top-4 left-4 bg-red-600 text-white px-4 py-1.5 rounded-lg text-xs font-bold shadow-md">{t.common.expired}</div>
                  )}
                </div>
                ...
                <div className="w-full md:w-[65%] p-6 md:p-8 flex flex-col">
                  ...
                  {promo.shortDesc && <p className="text-[#555555] text-[15px] mb-6 line-clamp-2 leading-relaxed">{localize(promo, 'shortDesc', locale)}</p>}
        ...
        <div className="text-center py-20 bg-white rounded-[24px] border border-[#E8E2D2]">
          ...
          <p className="text-[#888888] text-lg">{t.promotions.noPromotions}</p>
        </div>
      )}
    </div>
  </div>
